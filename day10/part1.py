...
start_and_end = defaultdict(set)


def get_num_paths_to_9(arr, start_r, start_c):
        ...
        for direction in directions.values():
            ...
            if isInBounds(arr, new_r, new_c) and arr[new_r][new_c] - curr_num == 1:
                ...
                if arr[new_r][new_c] == 9:
                    if (new_r, new_c) not in start_and_end[(start_r, start_c)]:
                        start_and_end[(start_r, start_c)].add((new_r, new_c))
                        total += 1
                else:
    ...
    for c in range(len(arr[0])):
        if arr[r][c] == 0:
            if (r, c) not in start_and_end:
                start_and_end[(r, c)] = set()
            answer += get_num_paths_to_9(arr, r, c)
